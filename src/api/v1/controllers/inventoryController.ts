...
const controller = {
    getAllInventories: async (req: Request, res: Response) => {
        try {
            ...
            const { filters, sort } = buildCommonQuery(req, ['productId', 'colorId', 'size']);

            const result = await inventoryService.getAllInventories(page, limit, filters, sort);
    ...
    getInventoryById: async (req: Request, res: Response) => {
        try {
            ...
            if (!inventory) {
                res.status(404).json({ success: false, code: 404, message: 'Inventory not found' });
                return
            }
    ...
    updateInventory: async (req: Request, res: Response) => {
        try {
            const data = req.body;
            const result = await inventoryService.updateInventory(req.params.id, data);
    ...
    getInventoriesByProductId: async (req: Request, res: Response) => {
        try {
            const data = await inventoryService.getInventoriesByProductId(req.params.productId);
            res.status(200).json({
                success: true,
                code: 200,
    ...
    getInventorySummary: async (req: Request, res: Response) => {
        try {
            const summary = await inventoryService.getInventorySummaryByProductId(req.params.productId);
            res.status(200).json({
                success: true,
                code: 200,
    ...
    increaseQuantity: async (req: Request, res: Response) => {
        try {
            ...
            const result = await inventoryService.increaseInventoryQuantity(req.params.id, amount);
    ...
    decreaseQuantity: async (req: Request, res: Response) => {
        try {
            const { amount } = req.body;
            const result = await inventoryService.decreaseInventoryQuantity(req.params.id, amount);
