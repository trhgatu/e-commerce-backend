...
export const checkoutCartToOrder = async (
  ...
  for (const cartItem of cart.items) {
    const inventoryDoc = await InventoryModel.findById(
      cartItem.inventoryId
    ).populate<{ productId: IProduct }>('productId');

    if (!inventoryDoc) {
      throw new Error(
        `Không tìm thấy tồn kho cho inventoryId ${cartItem.inventoryId}`
      );
    }
    ...
    if (inventory.quantity < cartItem.quantity) {
      throw new Error(
        `Không đủ tồn kho cho sản phẩm ${inventory.productId.name}`
      );
    }
